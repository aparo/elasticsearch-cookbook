...
def create_and_add_mapping(connection, index_name, type_name):
    ...
    connection.indices.put_mapping(index_name, type_name, {"_type": {"store": "yes"}, "type": "object", "properties": {
...
def populate(connection, index_name, type_name):
    ...
    connection.index(index=index_name, doc_type=type_name + "2", id=1, body={"name": "data1", "value": "value1"}, parent=1)
    connection.index(index=index_name, doc_type=type_name, id=2,
             body={"name": "Bill Baloney", "parsedtext": "Bill Testere nice guy", "uuid": "22222", "position": 2,
                   "date": datetime(2013, 12, 8)})
    connection.index(index=index_name, doc_type=type_name + "2", id=2, body={"name": "data2", "value": "value2"}, parent=2)
